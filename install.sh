...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/QYVORA/qyvora-aksum-cli/main/install.sh | bash
#   bash install.sh
#
# QYVORA OffSec
...
banner() {
    ...
    echo -e "  ${WHITE}AKSUM — Zero-Config Installer${NC}"
...
install_from_source() {
    ...
    if ! (cd "${WORK_DIR}/src" && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o "${WORK_DIR}/aksum-src" .); then
...
install_binary() {
    ...
    if [ "$dest" = "/usr/local/bin" ] && [ ! -w "$dest" ]; then
        if sudo -n true 2>/dev/null; then
            sudo -n install -m 0755 "$INSTALL_BIN" "$dest/aksum"
        else
    ...
    mkdir -p "$dest"
    if [ -w "$dest" ]; then
        install -m 0755 "$INSTALL_BIN" "$dest/aksum"
...
configure_path() {
    ...
    printf '\n# added by AKSUM installer\n%s\n' "$line" >> "$rc"
    warn "Added ${dest} to your PATH in ${rc}"
    warn "Run: source ${rc}  (or open a new terminal) to use 'aksum'"
}

# ---------------------------------------------------------------------------
# Desktop integration (Linux only): install the app icon and a .desktop entry
# so aksum shows up with its logo in GNOME's search/app grid. Data goes next
...
install_desktop_integration() {
    ...
    local icon_src="${WORK_DIR}/aksum.png"
    if [ -f "$PWD/assets/aksum.png" ]; then
        icon_src="$PWD/assets/aksum.png"
    elif [ ! -f "$icon_src" ] && [ -f "${WORK_DIR}/src/assets/aksum.png" ]; then
        icon_src="${WORK_DIR}/src/assets/aksum.png"
    fi
    ...
    install -m 0644 "$icon_src" "$icon_dir/aksum.png"

    local desktop="$apps_dir/aksum.desktop"
    if [ ! -f "$desktop" ]; then
        cat >"$desktop" <<EOF
[Desktop Entry]
Type=Application
Name=AKSUM
GenericName=Binary Security Assessment Platform
Comment=Binary security assessment & reverse-engineering platform
Exec=$dest/aksum
Icon=$icon_dir/aksum.png
Terminal=true
Categories=Utility;Development;Security;
Keywords=binary;elf;disassembly;reverse-engineering;security;analysis;findings;hardening;
EOF
    fi
    chmod 0644 "$desktop"
    ok "Installed desktop entry: ${desktop}"
    ok "Installed app icon: ${icon_dir}/aksum.png"
...
verify() {
    local dest="$1"
    local bin="$dest/aksum"

    if [ ! -x "$bin" ]; then
        warn "aksum binary not found at ${bin}."
        return 1
    fi

    info "Verifying installation..."
    "$bin" version 2>/dev/null || "$bin" --help >/dev/null 2>&1
    ok "AKSUM installed at ${bin}"
    ok "Run 'aksum --help' to get started."
    return 0
}
