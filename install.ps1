...
# it under %LOCALAPPDATA%\Programs\aksum\bin with the aksum icon, adds that
# directory to your user PATH, and creates a Start Menu shortcut — so aksum
...
#
# Options:
#   $env:AKSUM_VERSION    pinned release tag, e.g. v1.0.0 (default: latest)
#   $env:AKSUM_PREFIX     install directory (default: %LOCALAPPDATA%\Programs\aksum)
...
$Bin = "aksum"
$Version = $env:AKSUM_VERSION
$Prefix = $env:AKSUM_PREFIX
if (-not $Prefix) {
    $Prefix = Join-Path $env:LOCALAPPDATA "Programs\aksum"
}
...
function Build-FromSource {
    ...
    if (-not (Test-Path (Join-Path $PWD "go.mod"))) {
        ...
        $tmp = Join-Path $env:TEMP "qyvora-aksum-src"
        if (Test-Path $tmp) { Remove-Item -Recurse -Force $tmp }
        Write-Step "cloning QYVORA/qyvora-aksum..."
        git clone --depth 1 "https://github.com/QYVORA/qyvora-aksum" $tmp | Out-Null
        Set-Location $tmp
    }
    $go = Get-Command go -ErrorAction SilentlyContinue
    if (-not $go) { Write-Err "go is required to build from source"; exit 1 }
    go build -trimpath -ldflags "-s -w" -o (Join-Path $Dest "$Bin.exe") .
...
function Install-Icon {
    ...
    if (-not (Test-Path $ico)) {
        try {
            ...
            $candidates = @(
                (Join-Path $PWD "assets\aksum.ico"),
...
function Install-StartMenuShortcut {
    ...
    $sc.IconLocation = $ico
    $sc.Description = "binary reverse-engineering and vulnerability discovery for mobile & desktop apps"
    $sc.Save()
...
Write-Result "run 'aksum --help' to get started."
